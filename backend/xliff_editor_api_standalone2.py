...
TM_DATABASE = "translation_memory.db"
...
def init_tm_database():
    ...
    cursor.execute("""
        CREATE INDEX IF NOT EXISTS idx_source_text ON tm_entries(source_text)
    """)
    
    cursor.execute("""
        CREATE INDEX IF NOT EXISTS idx_languages ON tm_entries(source_lang, target_lang)
    """)
    
    conn.commit()
    conn.close()
    print("✅ Translation Memory database initialized")

def add_to_tm(source_text: str, target_text: str, source_lang: str, target_lang: str):
    """Add or update entry in Translation Memory"""
    ...
    cursor.execute("""
        SELECT id, usage_count FROM tm_entries
    ...
    if existing:
        ...
        entry_id, usage_count = existing
        cursor.execute("""
            UPDATE tm_entries
            SET target_text = ?, updated_at = CURRENT_TIMESTAMP, usage_count = ?
            WHERE id = ?
        """, (target_text, usage_count + 1, entry_id))
    else:
        # Insert new entry
        cursor.execute("""
            INSERT INTO tm_entries (source_text, target_text, source_lang, target_lang)
            VALUES (?, ?, ?, ?)
...
def search_tm(source_text: str, source_lang: str, target_lang: str, min_match: int = 70, limit: int = 5) -> List[Dict]:
    """
    Search Translation Memory for matches
    
    Returns list of matches with similarity percentage
    """
    ...
    # Get all entries for language pair
    cursor.execute("""
    ...
    matches = []
    for source, target, usage_count in entries:
        match_percent = fuzzy_match_ratio(source_text, source)
...
async def update_segment(
    ...
    # Save to Translation Memory (if target is not empty)
    if request.target and request.target.strip():
        try:
            add_to_tm(
                segment["source"],
                request.target,
            ...
            print(f"💾 Saved to TM: {segment['source'][:50]}... → {request.target[:50]}...")
        except Exception as e:
            print(f"⚠️  Failed to save to TM: {e}")
...
@app.post("/api/editor/tm/search", tags=["Translation Memory"])
async def search_translation_memory(
    source_text: str = Body(..., description="Source text to search for"),
    source_lang: str = Body(..., description="Source language code"),
    target_lang: str = Body(..., description="Target language code"),
    min_match: int = Body(70, description="Minimum match percentage (0-100)"),
    limit: int = Body(5, description="Maximum number of results")
):
    """
    ## Search Translation Memory
    
    Find translation matches from Translation Memory database.
    
    Returns:
    - **100% matches**: Exact matches from previous translations
    - **Fuzzy matches**: Similar segments (70-99% similarity)
    ...
    **Example Response:**
    ...
    """
    try:
        matches = search_tm(source_text, source_lang, target_lang, min_match, limit)
        
        print(f"🔍 TM Search: '{source_text[:50]}...' → Found {len(matches)} matches")
